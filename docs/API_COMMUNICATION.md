...
Use [`api.server.ts`](app/lib/api.server.ts:1) in loaders and actions:
...
export async function loader({}: Route.LoaderArgs) {
  ...
  // Uses Docker network: http://api:9999
...
export async function action({ request }: Route.ActionArgs) {
  ...
  await apiServer.createTask({ name });
  return { success: true };
}
```

### 2. Client-Side Interactions (Optional)

If you need client-side API calls (e.g., for real-time updates without page refresh):
...
export default function Tasks() {
  ...
  useEffect(() => {
    ...
    // Uses VITE_API_URL (public URL)
    apiClient.getTasks().then(setTasks);
  }, []);

  const handleCreate = async (name: string) => {
    await apiClient.createTask({ name });
    // Refresh list
    const updated = await apiClient.getTasks();
    setTasks(updated);
  };
...
### 3. Recommended: Form-Based Mutations

React Router's Form component handles mutations on the server automatically:
